...
pnpm test                     # Jest，全部测试
pnpm run test:coverage        # 必须 100% 覆盖 src/core/{stt,packet}.ts 和 src/core/danmaku/**，否则 CI 失败
...
| `src/core/danmaku/` | 弹幕录制 JSONL 解析、过滤管线、滚动轨道分配、ASS 渲染（设计见 `llmdoc/feat-ass-convert.md`） | 否（纯逻辑，fs 在 CLI 层） |
